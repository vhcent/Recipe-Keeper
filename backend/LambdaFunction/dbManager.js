...
function addRecipe(name, URL, userID, photo, recipeID) {
    return new Promise(function (resolve, reject) {
        connection.query(
            `select exists(select * from Saved where RecipeID = "${recipeID}" and UserID = "${userID}");`,
            (error, result, fields) => {
                ...
                //Second API call made inside a callback function bc it depends on the result
                    ...
                    connection.query(
                        `insert into Saved (Name, URL, UserID, Photo, RecipeID) values("${name}", "${URL}", "${userID}", "${photo}", ${recipeID});`,
                        (error, result) => {
                            if (error) {
                                console.log("Error", error);
                                reject(error);
                            ...
                            resolve(
                                resultGen(
                                    200,
                                    `added recipe ${name} with URL ${URL} and recipeID "${recipeID}"`
                                )
                            );
                            console.log(
    ...
    return new Promise(function (resolve, reject) {
        connection.query(
            ...
            (error, result) => {
                ...
                console.log(
                    `Updated Recipe Notes"${notes}" where id = "${ID}"`
                );
                resolve(
                    resultGen(200, `updated notes of id ${ID} with ${notes}`)
                );
            }
        );
    });
...
function getAllRecipes(userID) {
    return new Promise(function (resolve, reject) {
        connection.query(
            `select * from Saved where UserID = "${userID}";`,
...
function deleteRecipe(ID) {
    return new Promise(function (resolve, reject) {
        connection.query(`delete from Saved where ID = ${ID};`, (error) => {
            if (error) {
                console.log("Error", error);
                reject(error);
            } else {
...
*/
    ...
    return new Promise(function (resolve, reject) {
        connection.query(
            `SELECT RecipeID FROM Saved WHERE RecipeID IN (${recipeIDList}) AND UserID = "${userID}";`,
            (error, result) => {
                if (error) {
                    console.log("Error", error);
                    reject(error);
...
function addGrocery(userID, item) {
    return new Promise(function (resolve, reject) {
        connection.query(
            `insert into Grocery (Item, UserID) values("${item}", "${userID}");`,
            (error) => {
                if (error) {
                    console.log("Error", error);
                    reject(error);
...
function deleteGrocery(ID) {
    return new Promise(function (resolve, reject) {
        connection.query(`delete from Grocery where ID = ${ID};`, (error) => {
            if (error) {
                console.log("Error", error);
                reject(error);
            } else {
...
function getRecent(userID) {
    return new Promise(function (resolve, reject) {
        connection.query(
            `Select * from Recent where UserID = "${userID}";`,
            (error, result) => {
                if (error) {
                    console.log("Error", error);
                    reject(error);
...
function updateRecent(name, URL, userID, photo, recipeID) {
    return new Promise(function (resolve, reject) {
        connection.query(
            `select exists(select * from Recent where RecipeID = "${recipeID}" and UserID = "${userID}");`,
            (error, result, fields) => {
                ...
                if (res) {
                    connection.query(
                        `Select Position from Recent WHERE RecipeID = ${recipeID} and UserID = "${userID}";`,
                        (error, result) => {
                            ...
                            if (error) {
                                ...
                                connection.query(
                                    `UPDATE Recent SET Position = Position + 1 WHERE UserID = "${userID}" AND Position < ${posTop};`,
                                    (error, result) => {
                                        if (error) {
                                            console.log("Error", error);
                                            reject(error);
                                        ...
                                        connection.query(
                                            `Update Recent set Position = 1 where UserID = "${userID}" and RecipeID = ${recipeID};`,
                                            (error, result) => {
                                                ...
                                                if (error) {
                                                    ...
                                                    resolve(
                                                        resultGen(
                                                            200,
                                                            `Successfully updated the recents`
                                                        )
                                                    );
                                                }
                                            }
                                                ...
                                                (error, result, fields) => {
                                                    ...
                                                    if (error) {
                                                        ...
                                                        if (
                                                                ...
                                                                (
                                                                    ...
                                                                    if (error) {
                                                                        ...
                                                                        resolve(
                                                                            resultGen(
                                                                                200,
                                                                                "Updated recents, max recents reached"
                                                                            )
                                                                        );
                                                                    }
                                                                }
                                                            ...
                                                            console.log(
                                                                "should send back 200"
                                                            );
                                                            resolve(
                                                                resultGen(
                                                                    200,
                                                                    "Updated recents before max recents reached"
                                                                )
                                                            );
                                                        }
                                                    }
